...
const Analyze = () => {
  ...
  const exportRows = history.map((analysis, index) => ({
    ...
    burnDegree: analysis.burnDegree || analysis.burn_degree || 'Unknown',
    confidence: `${analysis.confidence ?? 0}%`,
    model: analysis.modelUsed || analysis.model_used || 'N/A',
    healingStage: analysis.healingStage || analysis.healing_stage || 'N/A'
  }));

  const getExportDocumentHtml = () => {
    const generatedAt = new Date().toLocaleString();
    const userName = user?.displayName || 'N/A';
    const userEmail = user?.email || 'N/A';
    const userId = user?.uid || 'N/A';
    const recordRows = exportRows.length > 0
      ? exportRows.map((row) => `
        <tr>
          <td>${row.serial}</td>
          <td>${row.date}</td>
          <td>${row.burnDegree}</td>
          <td>${row.confidence}</td>
          <td>${row.model}</td>
          <td>${row.healingStage}</td>
        </tr>
      `).join('')
      : `
        <tr>
          <td colspan="6" style="text-align:center;color:#6b7280;padding:24px;">No diagnostic history available.</td>
        </tr>
      `;
    ...
    return `
      ...
      <head>
        ...
        <style>
          ...
          .top { display: flex; justify-content: space-between; gap: 16px; margin-bottom: 26px; }
          .meta-box {
          ...
          .meta-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 12px 18px; font-size: 13px; }
      ...
      <body>
        <div class="top">
          <div>
            ...
            <div class="subtle">Burn Scar AI - Medical Analysis Record</div>
          </div>
          <div class="subtle">Generated: ${generatedAt}</div>
        </div>
        <div class="meta-box">
          <div class="meta-grid">
            ...
            <div><strong>Total Records:</strong> ${exportRows.length}</div>
          </div>
        </div>
        <table>
          <thead>
        ...
        <footer>
          This report is generated by Burn Scar AI for informational support and should not replace clinical diagnosis.
        </footer>
      </body>
      </html>
    `;
  ...
  const handleExportCSV = () => {
    const header = ['#', 'Date & Time', 'Burn Degree', 'Confidence', 'Model Used', 'Healing Stage'];
    const csvRows = [
      header.join(','),
      ...exportRows.map((row) => [
        row.serial,
        `"${row.date.replace(/"/g, '""')}"`,
        `"${row.burnDegree.replace(/"/g, '""')}"`,
        `"${row.confidence.replace(/"/g, '""')}"`,
        `"${row.model.replace(/"/g, '""')}"`,
        `"${row.healingStage.replace(/"/g, '""')}"`
      ].join(','))
    ];
    ...
    link.download = `diagnostic-history-${new Date().toISOString().slice(0, 10)}.csv`;
    link.click();
          ...
          <div className="mb-6 animate-fadeIn">
            <div className="flex items-start justify-between gap-3">
              ...
              <button
                ...
                className="inline-flex items-center gap-2 rounded-lg bg-gradient-to-r from-medical-blue to-blue-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:from-blue-600 hover:to-blue-700 hover:shadow-md transition-all"
              >
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="w-full max-w-5xl bg-white rounded-xl shadow-2xl border border-gray-200 overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
              <div>
                <h3 className="text-xl font-semibold text-gray-900">Diagnostic History</h3>
                <p className="text-xs text-gray-500 mt-0.5">Preview before export</p>
              </div>
              <button
                onClick={() => setShowExportPreview(false)}
                className="w-8 h-8 rounded-md hover:bg-gray-100 flex items-center justify-center text-gray-600"
              >
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                </svg>
            ...
            <div className="p-6 max-h-[75vh] overflow-y-auto bg-gray-50">
              <div className="bg-white border border-gray-200 rounded-lg p-6">
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 border-b border-gray-200 pb-4 mb-4">
                  <div>
                    <h2 className="text-2xl font-bold text-blue-900">Diagnostic History</h2>
                    <p className="text-xs text-gray-500">Burn Scar AI - Medical Analysis Record</p>
                  </div>
                  <p className="text-xs text-gray-500">Generated: {new Date().toLocaleString()}</p>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm bg-blue-50/60 border border-blue-100 rounded-md p-3 mb-4">
                  <p><span className="font-semibold text-gray-800">User:</span> {user?.displayName || 'N/A'}</p>
                  <p><span className="font-semibold text-gray-800">Email:</span> {user?.email || 'N/A'}</p>
                  <p><span className="font-semibold text-gray-800">User ID:</span> {user?.uid || 'N/A'}</p>
                  <p><span className="font-semibold text-gray-800">Total Records:</span> {exportRows.length}</p>
                </div>
                  ...
                  <table className="min-w-full text-xs border border-gray-200">
                    <thead>
                      <tr className="bg-blue-50 text-gray-700">
                        ...
                        <th className="text-left px-3 py-2 border-b border-gray-200">Date & Time</th>
                        <th className="text-left px-3 py-2 border-b border-gray-200">Burn Degree</th>
                        <th className="text-left px-3 py-2 border-b border-gray-200">Confidence</th>
                        <th className="text-left px-3 py-2 border-b border-gray-200">Model Used</th>
                        <th className="text-left px-3 py-2 border-b border-gray-200">Healing Stage</th>
                      </tr>
                    </thead>
                    <tbody>
                      {exportRows.length > 0 ? (
                        exportRows.map((row) => (
                          <tr key={row.serial} className="odd:bg-white even:bg-gray-50/50">
                            <td className="px-3 py-2 border-b border-gray-100">{row.serial}</td>
                            <td className="px-3 py-2 border-b border-gray-100">{row.date}</td>
                            <td className="px-3 py-2 border-b border-gray-100">{row.burnDegree}</td>
                            <td className="px-3 py-2 border-b border-gray-100">{row.confidence}</td>
                            <td className="px-3 py-2 border-b border-gray-100">{row.model}</td>
                            <td className="px-3 py-2 border-b border-gray-100">{row.healingStage}</td>
                          </tr>
                        ))
                      ) : (
                        <tr>
            ...
            <div className="px-6 py-4 border-t border-gray-200 bg-white flex items-center justify-end gap-3">
              ...
              <button
                ...
                className="px-4 py-2 rounded-md border border-medical-blue text-medical-blue hover:bg-medical-blue/5 transition-colors"
              >
                Download CSV
              </button>
              <button
                onClick={handleExportPDF}
                className="px-4 py-2 rounded-md bg-medical-blue text-white hover:bg-blue-700 transition-colors"
              >
                Download PDF
              </button>
            </div>
